...
+ (JGProgressHUD *)showAutoDismissHUDWithMessage:(NSString*)message dismissDelay:(NSTimeInterval)delayTime{
    ...
    if (!_static_hud.superview) {
        [_static_hud showInView:MainWindow animated:YES];
    }
...
{
    return [self showHUDInView:MainWindow successMessage:successMessage dismissAfterDelay:1.0];
}

+ (JGProgressHUD *)showFailureAutoDismissHUDWithMessage:(NSString*)failureMessage;
{
    return [self showHUDInView:MainWindow failureMessage:failureMessage dismissAfterDelay:1.0];
}
